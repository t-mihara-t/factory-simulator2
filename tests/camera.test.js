...
const fs=require('node:fs'),vm=require('node:vm'),S=require('../dist/simulation.js'),O=require('../dist/operations.js');
...
test('multi-lane starter layouts fit the small map area without clipping building artwork',()=>{
 const context={YardSim:S,Image:class{},ResizeObserver:class{observe(){}},matchMedia:()=>({matches:false}),devicePixelRatio:2};vm.createContext(context);vm.runInContext(fs.readFileSync(require.resolve('../dist/world.js'),'utf8'),context);
 for(const counts of [[1,1,2,1],[2,2,2,2],[1,1,1,3]])for(const [width,height] of [[390,210],[700,130],[320,180]]){
  const s=S.create('coast',{tutorial:true});O.configureLayout(s,counts);const canvas={getContext:()=>({}),getBoundingClientRect:()=>({width,height}),addEventListener(){}},mini={getContext:()=>({}),addEventListener(){}};const world=new context.YardWorld(canvas,mini,()=>s,()=>{},()=>{});
  for(const b of s.buildings){const p=world.screen(b.x+1.5,b.y+1.5),z=world.camera.z;assert(p.x-130*z>=0&&p.x+130*z<=width);assert(p.y-190*z>=0&&p.y+128*z<=height);}
 }
});
