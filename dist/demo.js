...
const HOLD={layout:3,order:2.4,launch:1.8,funding:4,shipping:3.2,debrief:3.8};
...
function tick(s,d,seconds){
 ...
 if(GATES.includes(phase)){
  ...
  else if(phase==='shipping')r=O.shipNow(s,s.jobs.find(j=>j.status==='finished')?.id);
...
function pace(s,fastWait=true,manual=2){
 ...
 const urgent=s.jobs.some(j=>j.deadline-s.t<=10||j.status==='material_funds'||j.held)||s.buildings.some(b=>b.broken&&!b.repairRemaining);
 if(urgent||s.decision||s.cash<1000)return manual;
 if(!guided&&(s.jobs.some(j=>j.status==='finished')||s.offers.some(o=>o.expires-s.t<=10)))return manual;
 const onlyTravel=s.jobs.length>0&&s.jobs.every(j=>['procurement','transfer'].includes(j.status));
