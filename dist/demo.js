...
const HOLD={layout:3,order:2.4,launch:6,funding:4,shipping:6,debrief:4.4};
function create(){return {elapsed:0,phase:null,dwell:0,accumulator:0,paused:false,planned:false,finished:false,error:null};}
function tick(s,d,seconds){
 ...
 if(GATES.includes(phase)){
  if(phase==='launch'&&!d.planned&&d.dwell>=2.5){S.setRelease(s,s.tutorial.orderId,60);d.planned=true;return;}
  if(d.dwell<(HOLD[phase]||2))return;
  let r;
  if(phase==='layout')r=O.configureLayout(s,[1,1,2,1]);
  else if(phase==='order')r=O.acceptForPlanning(s,s.offers.find(o=>o.training)?.id);
  else r=O.tutorialContinue(s);
  if(!r?.ok){d.error=r?.message||'デモを再開できませんでした。';d.finished=true;}
  else if(phase==='debrief')d.finished=true;
  return;
 }
 const reservedWait=phase==='watch'&&!s.jobs.length&&s.orders.some(o=>!o.releasePending&&o.releaseAt>s.t);d.accumulator+=dt*(reservedWait?32:8);
...
function pace(s,fastWait=true,manual=2){
 ...
 if(onlyTravel||!s.jobs.length&&s.orders.some(o=>o.status==='active'&&!o.releasePending&&o.released<o.quantity&&o.releaseAt>s.t))return 8;
