...
function* calculate(state,options={}){
 ...
 const horizon=Math.min(1800,Math.max(600,...s.orders.filter(o=>o.status==='active').map(o=>o.deadline-origin+300)));
 s.offers=[];s.nextOffer=Number.MAX_SAFE_INTEGER;s.nextEvent=Number.MAX_SAFE_INTEGER;
 const rows=new Map(),seen=new Map();let ticks=0;
 function observe(){
  for(const j of s.jobs){
   let row=rows.get(j.id);
   if(!row){row={id:j.id,order:j.order,product:j.product,deadline:j.deadline-origin,spans:[],shipped:false};rows.set(j.id,row);seen.set(j.order,(seen.get(j.order)||0)+1);}
   ...
   const stage=j.stage,key=[kind,facility,stage,j.held].join(':');
   let last=row.spans[row.spans.length-1];
   if(!last||last.key!==key){if(last)last.end=s.t-origin;last={key,kind,facility,stage,start:s.t-origin,end:s.t-origin};row.spans.push(last);}
 ...
 for(const o of state.orders.filter(o=>o.status==='active')){
  ...
  for(let n=0;n<missing;n++)rows.set(o.id+'-pending-'+n,{id:o.id+'-pending-'+n,order:o.id,product:o.product,deadline:o.deadline-origin,spans:[],shipped:false});
 }
 const result=[...rows.values()];
 for(const row of result)if(!row.shipped)row.warning=s.failed?'予測中に資金不足':!state.releaseEnabled&&!row.spans.length?'自動着工が停止中':row.spans.some(x=>x.key.endsWith(':true'))?'手動退避中：出庫再開が必要':'人員・経路・保全・待ち枠を確認（30分先まで）';
 return {origin,horizon:Math.min(horizon,s.t-origin),rows:result,failed:s.failed,ticks,assumptions:'受注済みのみ・現在の人員と経路で予測。運搬、待ち枠、保管、材料費、給与、設備劣化、開始済み修理を反映。今後の商談・ランダムイベント・追加の修理操作は含みません。'};
}
