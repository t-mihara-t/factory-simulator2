...
for(const m of app.matchAll(/\$\('([a-z][a-z0-9-]+)'\)/g)){assert(html.includes(`id="${m[1]}"`)||app.includes(`id="${m[1]}"`),`Unknown control ${m[1]}`);}
for(const asset of ['assets/process-atlas-v02.png','assets/factory-buildings.png','assets/trains-v08.png','assets/worker-v08.png','assets/sunny-assembly.wav'])assert(fs.statSync(path.join(root,asset)).size>100);
